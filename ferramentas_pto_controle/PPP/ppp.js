 ...
 zips.forEach(zip => {
   ...
   test(name, async t => {
       await t
         ...
         // .skipJsErrors()
         //.wait(60000)
         // .expect(Selector("button").withText("Download dos resultados")).contains("onclick")
         // .click(Selector("button").withText("Download dos resultados"));
         .click(Selector("button").withAttribute("onclick", /window.open/))
         .catch((error) => {
            if(error.errMsg.match('tags is not defined')){
              console.log('Caught expected page error.')
              return;
            }
            throw error;
         });
